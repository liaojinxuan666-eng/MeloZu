...
Device::Device(VkInstance instance_, vk::PhysicalDevice physical_, VkSurfaceKHR surface,
    ...
    if (YuzuSettings::values.enable_nsight_aftermath && extensions.device_diagnostics_config) {
    ...
    has_broken_compute =
        ...
        !YuzuSettings::values.enable_compute_pipelines.GetValue();
...
void Device::RemoveUnsuitableExtensions() {
    ...
    if (YuzuSettings::values.renderer_shader_feedback.GetValue()) {
...
void Device::CollectPhysicalMemoryInfo() {
    ...
    if (!is_integrated) {
        ...
        if (YuzuSettings::values.vram_usage_mode.GetValue() != YuzuSettings::VramUsageMode::Aggressive) {
            // Account for resolution scaling in memory limits
            const size_t normal_memory = 6_GiB;
            const size_t scaler_memory = 1_GiB * YuzuSettings::values.resolution_info.ScaleUp(1);
