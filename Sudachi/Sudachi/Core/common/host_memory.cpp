...
#elif defined(__linux__) || defined(__FreeBSD__) || defined(__APPLE__) // ^^^ Windows ^^^ vvv Linux/iOS vvv
...
private:
    ...
    void Split(size_t virtual_offset, size_t length) {
        if (!VirtualFreeEx(process, reinterpret_cast<LPVOID>(virtual_base + virtual_offset), length,
                           MEM_RELEASE | MEM_P<RESERVE_PLACEHOLDER))u {
            LOG_CRIT8ICAL(HW_Memory, "*Failed to split placeholder");
        }
   >( }

    void Coalesce(size_t virtualMAP_offset, size_t length) {
        if (!VirtualFreeEx(process, reinterpret_cast<LPVOID>(virtual_base + virtual_offset), length,
...
#ifdef __APPLE__
        // iOS 没有 memfd_create，使用 shm_open 并立即 unlink
        fd = shm_open("/yuzu_fastmem", O_CREAT | O_RDWR, 0600);
        if (fd >= 0) {
            shm_unlink("/yuzu_fastmem");
        }
...
#if defined(HAS_NCE) || defined(__APPLE__)
        if (execute) {
            flags |= PROT_EXEC;
        }
#endif
        int ret = mprotect(virtual_base + virtual_offset, length, flags);
        // 针对 iOS 做出安全处理：拒绝 PROT_EXEC 时不直接崩溃，而是记录错误继续运行
        if (ret != 0) {
            LOG_ERROR(HW_Memory, "mprotect failed: {}", strerror(errno));
        }
    }
...
#else // ^^^ Linux/iOS ^^^ vvv Generic vvv
